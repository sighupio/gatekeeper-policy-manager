...
# We have to do this ugly think in order to apply conditionally the login decorator
# only when it is enabled from the oonfig.
...
def get_constraints():
  ...
  try:
    ...
    # For some reasing, the previous query returns a lot of objects that we are
    # not interested. We need to filter the ones that we do care about.
    constraints = []
    ...
    if len(constraints) > 0:
      ...
      return render_template("message.html",
                            ...
                            action="There are no Constraints defined",
                            )
