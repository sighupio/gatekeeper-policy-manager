...
Since `v0.5.2-rc7` it's also possible to deploy GPM using the [provided Helm Chart](./chart). There's no Helm repository yet, but you can download the chart folder and use it locally:
...
```bash
docker run -v ~/.kube/config:/home/gpm/.kube/config -p 8080:8080 quay.io/sighup/gatekeeper-policy-manager:v0.5.2-rc7
```
...
Since `v0.5.2-rc7` GPM has basic multi-cluster support when using a `kubeconfig` with more than one context. GPM will let you chose the context right from the UI.
