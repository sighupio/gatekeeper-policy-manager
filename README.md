...
helm upgrade --install --namespace gatekeeper-system --set image.tag=v1.0.1 --values my-values.yaml gpm/gatekeeper-policy-manager
```
...
```bash
docker run -v ~/.kube/config:/home/gpm/.kube/config -p 8080:8080 quay.io/sighup/gatekeeper-policy-manager:v1.0.1
```
...
Since `v1.0.1` GPM has basic multi-cluster support when using a `kubeconfig` with more than one context. GPM will let you chose the context right from the UI.
...
FROM quay.io/sighup/gatekeeper-policy-manager:v1.0.1
