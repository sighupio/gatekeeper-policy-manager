...
![GPM Release](https://img.shields.io/badge/GPM-v1.1.2-blue)
...
helm upgrade --install --namespace gatekeeper-system --set image.tag=v1.1.2 --values my-values.yaml gatekeeper-policy-manager gpm/gatekeeper-policy-manager
```
...
```bash
docker run -v ~/.kube/config:/home/gpm/.kube/config -p 8080:8080 quay.io/sighup/gatekeeper-policy-manager:v1.1.2
```
...
FROM quay.io/sighup/gatekeeper-policy-manager:v1.1.2
