...
- **More of GPM now requires a login.** When OIDC is on, the list of clusters (`/api/v1/contexts`), the mutations view and the events view need a session, which was not the case in 1.x. Only a small set of paths stay open: the health check, the auth-status endpoint, the login and logout routes, `/metrics`, and the static assets.
