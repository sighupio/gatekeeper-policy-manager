...
This version replaces the Python backend with a new one written in Go, rebuilds the web interface to render on the server instead of as a React single-page app, adds views for Gatekeeper mutations and events, adds support for Kubernetes 1.36 and updates all dependencies. The views work the same as before.
